...
async function AddUser(UserResult) {
  var factory = getFactory();
  var NS = 'org.kyc.network';
  var details = UserResult.bankid;
  
  var user = factory.newResource(NS, 'UserResult', UserResult.userid);
  user.bank = factory.newRelationship(NS, 'BankBalance', details.balanceid);
  user.status = 'Pending';
...
async function CheckKyc(CheckKycParameters) {
  ...
  var originalbankbalance = userdetails.bank;

  if( originalbankbalance.balanceid == bankdetails.balanceid ){
    return currentstatus+','+0;
  }
  else if( currentcost <= bankdetails.balance ){
    ...
    originalbankbalance.balance = originalbankbalance.balance + currentcost;
    const changeval2 = await getAssetRegistry('org.kyc.network.BankBalance');
    await changeval2.update(originalbankbalance);
    return currentstatus+','+currentcost;
  }
  else{
